...
async function loadState() {
  ...
  try { const r = await window.storage.get(STORE_KEY, true); if (r && r.value) expenses = JSON.parse(r.value); } catch (e) {}
  try { const r = await window.storage.get(NAMES_KEY, true); if (r && r.value) names = JSON.parse(r.value); } catch (e) {}
  try { const r = await window.storage.get(RATES_KEY, true); if (r && r.value) rates = JSON.parse(r.value); } catch (e) {}
  try { const r = await window.storage.get(SETTLE_KEY, true); if (r && r.value) settlements = JSON.parse(r.value); } catch (e) {}
  return { expenses, names, rates, settlements };
}
const save = (key, val) => { window.storage.set(key, JSON.stringify(val), true).catch((e) => console.error("save failed", e)); };
...
export default function App() {
  ...
  return (
    <div style={S.shell}>
      ...
      <p style={S.footnote}>Saved automatically and shared between you both.</p>
    </div>
  );
}
...
function OwedLine({ name, net }) {
  const owed = net > 0.005;     // this person is owed money
  const owing = net < -0.005;   // this person owes money
...
function SettleForm({ names, balance, rates, onClose, onSave }) {
  // Pre-select the direction and amount that would clear the outstanding balance.
  // balance positive => B owes A, so B paying A clears it.
